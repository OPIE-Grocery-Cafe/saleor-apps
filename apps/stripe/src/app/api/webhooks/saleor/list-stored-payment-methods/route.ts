...
import { listStoredPaymentMethodsDefinition } from "../stored-payment-methods/webhook-definitions";
import { withRecipientVerification } from "../with-recipient-verification";

const service = new StoredPaymentMethodsService(appConfigRepoImpl);
const handler = listStoredPaymentMethodsDefinition.createHandler(
  withRecipientVerification(async (_request, ctx) => {
    try {
      return Response.json(await service.list(webhookContext(ctx.authData, ctx.payload)));
    } catch (error) {
